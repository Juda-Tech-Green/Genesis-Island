...
export default class Player extends MatterEntity {
    constructor(data) {
        ...
        this.spriteWeapon.setScale(0.85)
        this.spriteWeapon.setOrigin(0.25, 0.75);
        this.spriteWeapon.setDepth(5)
        this.scene.add.existing(this.spriteWeapon);


        const { Body, Bodies } = Phaser.Physics.Matter.Matter;
        var playerCollider = Bodies.circle(this.x, this.y, 10, { isSensor: false, label: 'playerCollider' })
        var playerSensor = Bodies.circle(this.x, this.y, 25, { isSensor: true, label: 'playerSensor' });
    ...
    static preload(scene) {
        scene.load.atlas('female', 'assets/images/female.png',
